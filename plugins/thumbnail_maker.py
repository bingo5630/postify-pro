...
def crop_image(img, target_size, crop_state):
    ...
    if img_ratio > target_ratio:
        ...
        if crop_state == 0: left = (img.width - new_width) // 2
        elif crop_state == 1: left = 0
        else: left = img.width - new_width
...
async def generate_poster(anime_img_url=None, custom_image_path=None, title="", genres="", synopsis="", username="", logo_url=None, crop_state=0, small_caps=False):
    ...
    # ==========================================
    # FINAL BULLETPROOF HALO KILLER & HEXAGON MASK
    # Technique: Erosion (Shrinking the mask inward)
    # ==========================================
    
    # 1. Convert mask to L mode
    hex_mask_l = fetched_mask.convert('L')
    
    # 2. Strict Fix for leak: Ensure outside is absolutely black
    # fetched_mask has alpha issues. We use fetched_mask as its own mask to 
    # guarantee solid L data ONLY where there was solid artwork before.
    strict_l_canvas = Image.new('L', base_template.size, 0)
    strict_l_canvas.paste(hex_mask_l, (0, 0), fetched_mask)
    
    # 3. Apply Erosion (MinFilter) to shrink the white region by ~2 pixels.
    # This prevents ANY white halo bleeding from the edge.
    eroded_mask_l = strict_l_canvas.filter(ImageFilter.MinFilter(size=5))
    
    # 4. Strict Binary Threshold for sharp cut
    binary_mask = eroded_mask_l.point(lambda p: 255 if p > 128 else 0)

    anime_artwork = crop_image(anime_img, base_template.size, crop_state)
    
    # 5. Paste strictly restricted dynamic dynamic artwork on template
    ...
    except: font_title = font_genres = font_synopsis = font_brand = ImageFont.load_default()
